...
def TempSweepIV():
    ...
    tk.Label(iv_frame, text="Wait between IV curves [s]").grid(row=2, column=0, sticky='e')
    wait_entry = tk.Entry(iv_frame, width=10); wait_entry.insert(0, "0.0")
    wait_entry.grid(row=2, column=1, sticky='w')

    tk.Label(iv_frame, text="NPLC (empty=auto/default)").grid(row=2, column=2, sticky='e')
    nplc_entry = tk.Entry(iv_frame, width=14)
    nplc_entry.grid(row=2, column=3, sticky='w')

    tk.Label(iv_frame, text="Compliance V [V] (empty=auto)").grid(row=3, column=0, sticky='e')
    comp_v_entry = tk.Entry(iv_frame, width=14)
    comp_v_entry.grid(row=3, column=1, sticky='w')

    tk.Label(iv_frame, text="Compliance I [A] (empty=auto)").grid(row=3, column=2, sticky='e')
    ...
    def _configure_measurement(dev, nplc_txt, comp_v_txt, comp_i_txt):
        try:
            txt = (nplc_txt or '').strip()
            if txt:
                dev.smua.measure.nplc = float(txt)
        except Exception:
            pass
        try:
            txt = (comp_v_txt or '').strip()
            if txt:
                dev.smua.source.limitv = float(txt)
        except Exception:
            pass
        try:
            txt = (comp_i_txt or '').strip()
            if txt:
                dev.smua.source.limiti = float(txt)
        except Exception:
            pass
    ...
    def _fmt_temp_range_from_values(temp_values):
        vals = np.array(temp_values, dtype=float)
        t_lo = float(np.min(vals))
        t_hi = float(np.max(vals))
        return f"{t_lo:.4g}K-{t_hi:.4g}K"

    def _save_curve_incremental(base_dir, smu_name, idx, times, xs, ys, temp_values, user_base):
        os.makedirs(base_dir, exist_ok=True)
        range_tag = _fmt_temp_range_from_values(temp_values)
        stamp = datetime.datetime.now().strftime('%Y%m%d_%H%M%S')
        single_path = os.path.join(base_dir, f"{user_base}_{smu_name}_{range_tag}_{stamp}_curve_{idx:05d}.txt")
        table = np.column_stack([np.array(times, dtype=object), np.array(xs, dtype=float), np.array(ys, dtype=float), np.array(temp_values, dtype=float)])
    ...
    def _run(base_smu1, base_smu2):
        try:
            ...
            if use_smu1.get():
                ...
                _configure_measurement(k, nplc_txt, comp_v_txt, comp_i_txt)
            if use_smu2.get():
                _set_sense(k2, bool(smu2_4wire.get()))
                _configure_measurement(k2, nplc_txt, comp_v_txt, comp_i_txt)
            ...
            run_folder_primary_smu1 = os.path.join(SAVE_PATH, get_filename(base_smu1)) if use_smu1.get() else None
            run_folder_backup_smu1 = os.path.join(SAVE_PATH_2, get_filename(base_smu1)) if use_smu1.get() else None
            run_folder_primary_smu2 = os.path.join(SAVE_PATH, get_filename(base_smu2)) if use_smu2.get() else None
            run_folder_backup_smu2 = os.path.join(SAVE_PATH_2, get_filename(base_smu2)) if use_smu2.get() else None
            for pth in [run_folder_primary_smu1, run_folder_backup_smu1, run_folder_primary_smu2, run_folder_backup_smu2]:
                if pth:
            ...
            smu1_cbar, smu2_cbar = None, None
            temp_min_seen, temp_max_seen = None, None
            if use_smu1.get() and use_smu2.get():
                ...
                if smu1_ax.get_legend() is not None:
                    smu1_ax.get_legend().remove()
                if smu2_ax.get_legend() is not None:
                    smu2_ax.get_legend().remove()

            def _update_temp_colors():
                nonlocal smu1_cbar, smu2_cbar, temp_min_seen, temp_max_seen
                if temp_min_seen is None or temp_max_seen is None:
                    return
                t_lo = float(temp_min_seen)
                t_hi = float(temp_max_seen)
                if t_hi <= t_lo:
                    t_hi = t_lo + 1e-9
                norm = mcolors.Normalize(vmin=t_lo, vmax=t_hi)
            ...
            for sec_idx, (target_t, ramp, regen) in enumerate(sections):
                ...
                while abs(float(client.query('T_sample.kelvin') or 300.0) - target_t) > target_tol:
                    ...
                    if use_smu1.get() and use_smu2.get():
                        if alt['next'] == 1:
                            ...
                            curve_idx['SMU1'] += 1
                            _save_curve_incremental(run_folder_primary_smu1, "SMU1", curve_idx['SMU1'], times, xs, ys, [T_now] * len(xs), base_smu1)
                            _save_curve_incremental(run_folder_backup_smu1, "SMU1", curve_idx['SMU1'], times, xs, ys, [T_now] * len(xs), base_smu1)
                            if cur_ax is not None:
                                ...
                                temp_min_seen = T_now if temp_min_seen is None else min(temp_min_seen, T_now)
                                temp_max_seen = T_now if temp_max_seen is None else max(temp_max_seen, T_now)
                                smu1_ax.scatter(px, py, s=8, c=np.full(len(px), T_now, dtype=float), cmap=cmap, alpha=0.9)
                                smu1_ax.set_xlabel(xl); smu1_ax.set_ylabel(yl)
                                _update_temp_colors()
                            alt['next'] = 2
                            ...
                            curve_idx['SMU2'] += 1
                            _save_curve_incremental(run_folder_primary_smu2, "SMU2", curve_idx['SMU2'], times, xs, ys, [T_now] * len(xs), base_smu2)
                            _save_curve_incremental(run_folder_backup_smu2, "SMU2", curve_idx['SMU2'], times, xs, ys, [T_now] * len(xs), base_smu2)
                            if cur_ax is not None:
                                if swap_axes_state['value']:
                                    px, py = ys, xs
                                    xl, yl = 'Voltage(V)', 'Current(A)'
                                ...
                                temp_min_seen = T_now if temp_min_seen is None else min(temp_min_seen, T_now)
                                temp_max_seen = T_now if temp_max_seen is None else max(temp_max_seen, T_now)
                                smu2_ax.scatter(px, py, s=8, c=np.full(len(px), T_now, dtype=float), cmap=cmap, alpha=0.9)
                                smu2_ax.set_xlabel(xl); smu2_ax.set_ylabel(yl)
                                _update_temp_colors()
                            alt['next'] = 1
                            if wait_between_curves_s > 0: time.sleep(wait_between_curves_s)
                    elif use_smu1.get():
                ...
                if rt1 is not None and len(rt1.data) > 0:
                    ...
                    if len(new_rows_1) > 0:
                        ...
                        temps_1 = d1[:, 3].astype(float)
                        safe_file(d1, f"{base_smu1}_SMU1_{_fmt_temp_range_from_values(temps_1)}")
                ...
                if rt2l is not None and len(rt2l.data) > 0:
                    ...
                    if len(new_rows_2) > 0:
                        ...
                        temps_2 = d2[:, 3].astype(float)
                        safe_file(d2, f"{base_smu2}_SMU2_{_fmt_temp_range_from_values(temps_2)}")
            ...
            _ui_call(lambda txt=f"Done. Incremental files in: {run_folder_primary_smu1 or run_folder_primary_smu2} and/or {run_folder_primary_smu2 or run_folder_primary_smu1}": status.config(text=txt))
        except Exception as ex:
