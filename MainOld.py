...
DEFAULTS = {
    ...
    "burst_ops_before_wait":     0,   # wie viele Kommandos vor waitcomplete()
...
def _order_center_out_around_start(uA_points, start_mA):
    ...
    ordered = sorted(uniq, key=keyfun)
    return ordered
...
def _junction_pulse_measure_v(I_uA, pulse_width_s, pulse_settle_s, cooldown_s):
    ...
    I_A = float(I_uA) * 1e-6
    try:
        # Modus/Limit setzen (idempotent) und Ausgang EIN
        k.smua.source.func   = 2           # current
        k.smua.source.limitv = 0.01
        k.smua.source.output = 1
        _maybe_wait(k, which='k')
    except Exception as ex:
        if not _recover_queue_full(k, ex, which='k'):
            raise
        # retry
    ...
    try:
        ...
        if _recover_queue_full(k, ex, which='k'):
            v = k.smua.measure.v()
        else:
            v = float('nan')
...
try:
    ...
    def _find_extrema_centered(x, y, min_prom_rel=0.05):
        ...
        if len(y) > 0:
            ...
            def prominent(idxs, invert=False):
                keep = []
                for idx in idxs:
                    val = y[idx]
                    left = y[max(0, idx-1)]; right = y[min(len(y)-1, idx+1)]
                    prom = (val - min(left, right)) if not invert else (max(left, right) - val)
                    if prom >= thr: keep.append(idx)
                return keep
            maxs = prominent(maxs, invert=False)
            mins = prominent(mins, invert=True)
        return mins, maxs
...
def _coil_set_current_uA(target_uA, ramp_steps, settle_s):
    ...
    try:
        k2.smua.source.func   = 2  # current
        k2.smua.source.limitv = 10.0
        k2.smua.source.output = 1
        _maybe_wait(k2, which='k2')
    except Exception as ex:
        if not _recover_queue_full(k2, ex, which='k2'):
            raise
    ...
    if ramp_steps <= 1:
        try:
            k2.smua.source.leveli = target_uA * 1e-6
            _maybe_wait(k2, which='k2')
        except Exception as ex:
            if not _recover_queue_full(k2, ex, which='k2'):
                raise
        ...
        for frac in np.linspace(0.0, 1.0, ramp_steps):
            try:
                k2.smua.source.leveli = (frac * target_uA) * 1e-6
                _maybe_wait(k2, which='k2')
            except Exception as ex:
                if not _recover_queue_full(k2, ex, which='k2'):
                    raise
...
def _find_ic_uA_guided(
    ...
    *, sign=+1, collect_iv=False, stop_event=None,
    ...
    def pulse(IuA):
        ...
        if collect_iv: iv.append((sign * IuA, v))
        return v
...
def Fraunhofer_Pattern():
    ...
    def start_clicked():
        ...
        if not base:
            messagebox.showerror("Error", "Please provide a base filename."); return
        ...
        try:
            settings = dict(
                ...
                measure_negative  = bool(frame._neg_var.get()),
        ...
        status_lbl.config(text=f"Starting… T0 = {_ff_state.T0:.3f} K")
        _ff_state.thread = threading.Thread(target=_worker_run_log, args=(_ff_state,), daemon=True)
        _ff_state.thread.start()
        _poll_queues(_ff_state, status_lbl, bar, ic_line_pos, ic_line_neg, ic_ax, ic_canvas,
                     iv_line, iv_ax, iv_canvas)

    def stop_clicked():
        if _ff_state and _ff_state.thread and _ff_state.thread.is_alive():
            _ff_state.stop_event.set()
            status_lbl.config(text="Stopping… (saving partial data)")

        # Buttons zurücksetzen (erneut startbar)
...
def MagneticMeasurementLinear():
    ...
    def apply_step():
        try:
            val = float(step_entry.get())
            if _ff_state and _ff_state.thread and _ff_state.thread.is_alive():
                _ff_state.command_q.put({"op": "set_step", "step_mA": val})
                status_lbl.config(text=f"Live: new step {val} mA queued")
        except Exception as ex:
            messagebox.showerror("Step error", str(ex))

    tk.Button(live, text="Apply step", width=12, command=apply_step).grid(row=0, column=2, padx=6, pady=2)
    ...
    def do_jump():
        try:
            val = float(jump_entry.get())
            if _ff_state and _ff_state.thread and _ff_state.thread.is_alive():
                _ff_state.command_q.put({"op": "jump", "target_mA": val})
                status_lbl.config(text=f"Live: jump to {val} mA queued")
        except Exception as ex:
            messagebox.showerror("Jump error", str(ex))

    tk.Button(live, text="Jump", width=12, command=do_jump).grid(row=1, column=2, padx=6, pady=2)
    ...
    def start_clicked():
        ...
        start_btn.config(state=tk.DISABLED); stop_btn.config(state=tk.NORMAL)
        status_lbl.config(text=f"Starting (linear)… T0 = {_ff_state.T0:.3f} K")
    ...
    def start_clicked():
        ...
        try:
            settings = dict(
                ...
                pulse_settle_s    = float(e["pulse_settle_ms"].get()) * 1e-3,
                cooldown_s        = float(e["cooldown_ms"].get())     * 1e-3,
                ic_start_uA       = float(DEFAULTS["ic_start_uA"]),   # schnell
                ...
                measure_negative  = False,  # rough: nur |Ic| (schnell)
                error_drain_every_pulses = int(DEFAULTS["error_drain_every_pulses"]),
                burst_ops_before_wait = int(DEFAULTS["burst_ops_before_wait"]),
                burst_backoff_s       = float(DEFAULTS["burst_backoff_s"]),
            )
        ...
        start_btn.config(state=tk.DISABLED); stop_btn.config(state=tk.NORMAL)
        status_lbl.config(text=f"Starting (rough)… T0 = {_ff_state.T0:.3f} K")
        _ff_state.thread = threading.Thread(target=_worker_run_rough,
                                            args=(_ff_state, ic_line_pos, ic_ax, ic_canvas, status_lbl, bar),
                                            daemon=True)
        _ff_state.thread.start()
        _poll_queues(_ff_state, status_lbl, bar, ic_line_pos, ic_line_neg, ic_ax, ic_canvas,
                     None, None, None)
...
def _build_signed_pair_plan_from(current_uA, max_mA, step_mA, *, include_negative=True):
    ...
    def add(v):
        ...
        if abs(v) <= max_uA + 1e-9 and key not in seen:
            seq.append(float(v)); seen.add(key)
...
def run_sweep(sweep_points, user_input, user_input2=None):
    ...
    def update_iv_plot2(_):
        if voltage_data2:
            line2.set_data(voltage_data2, current_data2)
...
def create_sweep():
    ...
    tk.Button(controls_frame, text="Sweep start", command=run).pack(side=tk.RIGHT)
...
def IV_Temp():
    ...
    def add_iv_section():
        ...
        iv_section_frames.append(frame)

    def add_temp_section():
        nonlocal current_temp_row, current_temp_col
        frame = create_temp_frame(temp_sections_frame)
        frame.grid(row=current_temp_row, column=current_temp_col, padx=10, pady=10)
        temp_section_frames.append(frame)
        current_temp_row += 1
        if current_temp_row >= 10:
            current_temp_row = 0
            current_temp_col += 1

    # Initialize row and column index for temperature sections
    current_temp_row = 0
    current_temp_col = 0
    ...
    def run():
        ...
        sweep_data = []
        for frame in iv_section_frames:
            try:
                start_value = frame.winfo_children()[1].get()
                end_value = frame.winfo_children()[3].get()
                steps_value = frame.winfo_children()[5].get()
        ...
        # Collect temperature data (start_temp, end_temp, ramp_speed) from the GUI
        temperature_data = []
        for frame in temp_section_frames:
            try:
    ...
    add_iv_section()  # Add one section to begin with for IV sweep data

    temp_sections_frame = tk.Frame(iv_temp_window)
    temp_sections_frame.pack()

    add_temp_section()  # Add one section to begin with for temperature data
    ...
    tk.Button(controls_frame, text="Add Section (Temp)", command=add_temp_section).pack(side=tk.LEFT)
...
def IV_During_Cooldown():
    """
    Kontinuierliche IV-Sweeps während des Cooldowns bis zu einer Endtemperatur.
    Messmodus: 2-Punkt (SMU1 lokal), zusätzlich Sense-Spannung über SMU2.
    Nach jedem Sweep wird separat gespeichert inkl. gemessenem Temperaturbereich.
    """
    global measurement
    measurement = 1

    TEMP_TOLERANCE_K = 0.05
    MIN_END_TEMP_K = 3.3
    MAX_TEMP_K = 290.0
    ...
    def run_single_iv_sweep_with_sense(sweep_points, use_smu2_sense):
        ...
        if use_smu2_sense:
            rt2 = ResultTable(
                column_titles=['Time', 'Voltage_Source[V]', 'SenseVoltage[V]', 'Temperature[K]'],
                units=[' ', 'V', 'V', 'K'],
        ...
        # 2-Punkt auf SMU1
        k.smua.sense = k.smua.SENSE_LOCAL
        # SMU2 optional als Voltmeter (0 A Source)
        if use_smu2_sense:
            k2.smua.sense = k2.smua.SENSE_LOCAL
        ...
        for start, end, steps in sweep_points:
            ...
            for v_set in voltages:
                ...
                v_source = float(k.smua.measure.v())
                if use_smu2_sense:
                    v_sense = float(k2.smua.measure.v())

                rt.append_row([ts, v_source, i_meas, T])
                if use_smu2_sense:
                    rt2.append_row([ts, v_source, v_sense, T])
    ...
    def run_worker(sweep_points, target_end_temp, ramp_speed, pause_s, user_input, use_smu2_sense):
        try:
            # Cooldown starten
            temperature_control.start((float(target_end_temp), float(ramp_speed)))
            sweep_index = 1
            ...
            while measurement == 1:
                ...
                filename_main = f"{user_input}{suffix}"
                filename_sense = f"{user_input}{suffix}_sense"

                data_numeric = np.array([[ts] + list(map(float, vals)) for ts, *vals in rt.data])
                safe_file(data_numeric, filename_main)
                if use_smu2_sense and rt2:
                    data_numeric2 = np.array([[ts] + list(map(float, vals)) for ts, *vals in rt2.data])
                    safe_file(data_numeric2, filename_sense)

                print(f"Sweep {sweep_index} gespeichert ({t_min:.3f} K bis {t_max:.3f} K).")
                sweep_index += 1

                if _query_sample_temperature() <= (target_end_temp + TEMP_TOLERANCE_K):
                    break
    ...
    def run():
        ...
        try:
            target_end_temp = float(end_temp_entry.get())
        ...
        if target_end_temp < MIN_END_TEMP_K:
            Errormessage("Endtemperatur muss mindestens 3.3 K sein.")
            target_end_temp = MIN_END_TEMP_K
            end_temp_entry.delete(0, tk.END)
            end_temp_entry.insert(0, str(MIN_END_TEMP_K))
        elif target_end_temp > MAX_TEMP_K:
            Errormessage("Endtemperatur außerhalb des Bereichs (3.3-290 K).")
            target_end_temp = MAX_TEMP_K
            end_temp_entry.delete(0, tk.END)
            end_temp_entry.insert(0, str(MAX_TEMP_K))
        ...
        threading.Thread(
            target=run_worker,
            args=(sweep_points, target_end_temp, ramp_speed, pause_s, user_input, use_smu2_sense_var.get() == 1),
            daemon=True
        ).start()
    ...
    tk.Label(settings_frame, text="End Temperature [K] (min 3.3)").grid(row=0, column=0, sticky="w")
    end_temp_entry = tk.Entry(settings_frame, width=10)
    end_temp_entry.grid(row=1, column=0, padx=(0, 10), sticky="w")
    end_temp_entry.insert(0, "3.3")

    tk.Label(settings_frame, text="Ramp Speed [K/min]").grid(row=0, column=1, sticky="w")
    ramp_speed_entry = tk.Entry(settings_frame, width=10)
    ramp_speed_entry.grid(row=1, column=1, padx=(0, 10), sticky="w")
    ramp_speed_entry.insert(0, "1")

    tk.Label(settings_frame, text="Pause between sweeps [s]").grid(row=0, column=2, sticky="w")
    pause_entry = tk.Entry(settings_frame, width=10)
    pause_entry.grid(row=1, column=2, sticky="w")
...
def create_4ptc_measurement():
    ...
    def get_tc_data():
        ...
        def update_plot(_):
            ...
            line.set_data(temperature_data, resistance_data)
            ax.relim()
            ax.autoscale_view()
            fig.canvas.flush_events()
        ani = FuncAnimation(fig, update_plot, frames=10000, repeat=False, interval=5000)
        ani._start()
        for start_temp, end_temp, ramp_speed, ramp_current in tc_data:
            # Set initial temperature
            if start_temp+5<temperature_control.kelvin and start_temp >5:
            ...
            if measurement_started.get() == 1:
                ...
                while abs(client.query('T_sample.kelvin') - start_temp) > 0.1:
                    voltage = k2.smua.measure.v()  # Measure the voltage
                    voltage2 = k.smua.measure.v()
                    timestamp = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')  # Get the current time
                    temperature = temperature_control.kelvin # Get the current temperature
                    temperature_data.append(temperature)
                    resistance_data.append(voltage / ramp_current)  # Calculate resistance from voltage and current               
                    rt.append_row([timestamp,ramp_current, voltage, temperature])  # Append data to the ResultTable
    ...
    def get_tc_data():
        ...
        for start_temp, end_temp, ramp_speed, ramp_current in tc_data:
            ...
            if measurement_started.get() == 1:
                ...
                while abs(client.query('T_sample.kelvin') - start_temp) > 0.1:
                    current, voltage = k.smua.measure.iv()  # Measure the voltage
                    timestamp = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')  # Get the current time
                    temperature = temperature_control.kelvin # Get the current temperature
                    Temp_data.append(temperature)
                    rt.append_row([timestamp, current,voltage, temperature])  # Append data to the ResultTable
...
def Find_JJ_at_Several_Temperatures():
    ...
    def _worker(run_cfg):
        try:
            ...
            for _, dev, _ in devices:
                _conf_smu(dev, nplc=run_cfg["nplc"], vlimit_V=run_cfg["vlimit"])
            ...
            for t_idx, target_t in enumerate(temps):
                ...
                reached_t = _wait_for_target_temperature(
                    target_t,
                    run_cfg["ramp_k_min"],
                    run_cfg["t_tol"],
                    run_cfg["reload_timer_s"],
                    ui_status,
                    stop_event
                )
                ...
                _dual_save_csv_and_txt(
                    run_cfg["base"], f"_T{temp_tag}K_JJtemp",
                    ("timestamp", "T_K", "device", "cycle", "section", "I_set_A", "I_meas_A", "V_meas_V"),
                    this_temp_rows
                )
                for dev_name, _, style in devices:
                    ui_add_legend_entry(dev_name, style, color, reached_t, len(temps) <= 14)
    ...
    def _start():
        if stop_btn.cget("state") == tk.NORMAL:
            return
        try:
            use_smua, use_smub = bool(use1.get()), bool(use2.get())
            if not (use_smua or use_smub):
                messagebox.showerror("Find JJ T-sweep", "Please select at least one SMU.")
                return
            ...
            for fr in section_frames:
                ...
                sections.append((start_uA, end_uA, max(2, steps)))
            if not sections:
                messagebox.showerror("Find JJ T-sweep", "No valid current sweep section configured.")
                return
            ...
            run_cfg = dict(
                ...
                ramp_k_min=ramp_k_min,
                t_tol=t_tol,
                settle_after_t_s=settle_after_t_s,
                reload_timer_s=reload_timer_s,
                nplc=nplc,
                vlimit=vlimit,
            ...
            messagebox.showerror("Find JJ T-sweep", f"Invalid input: {ex}")
            return

        stop_event.clear()
        ui_status("Starting…")
        ui_buttons(False, True)
        threading.Thread(target=_worker, args=(run_cfg,), daemon=True).start()
...
add_button_with_details(tc_frame, "IV During Cooldown", IV_During_Cooldown, "Kontinuierlicher 2-Punkt IV Sweep während Cooldown bis Endtemperatur (min. 3.3 K). Optional kann SMU2 für zusätzliche Sense-Spannung genutzt werden; jeder Sweep wird separat mit Temperaturbereich gespeichert.")
